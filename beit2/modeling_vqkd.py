...
class VQKD(nn.Module):
    ...
    def __init__(self,
        ...
        self.token_shape = (encoder_config['img_size'] // self.patch_size,
        ...
        if self.teacher_model_type == 'clip':
            ...
            self.teacher_model, _ = clip.load(
                "ViT-B/16", device='cpu', jit=False)
    ...
    def no_weight_decay(self):
        return {
            'quantize.embedding.weight', 'decoder.cls_token',
    ...
    def get_regress_target(self, x, **kwargs):
        ...
        if self.teacher_model_type == 'clip':
            target = self.teacher_model.encode_image(
                norm_imgs,
                return_all_tokens=True) @ self.teacher_model.visual.proj
        elif self.teacher_model_type == 'dino':
            target = self.teacher_model.forward(
                norm_imgs, return_patch_tokens=True)
        else:
...
class ScalingLayerForClip(nn.Module):
    ...
    def __init__(self):
        ...
        self.register_buffer(
            'shift',
            torch.Tensor([0.48145466, 0.4578275, 0.40821073])[None, :, None,
                                                              None])
        self.register_buffer(
            'scale',
            torch.Tensor([0.26862954, 0.26130258, 0.27577711])[None, :, None,
                                                               None])
...
@register_model
def vqkd_encoder_base_decoder_1x768x12_clip(pretrained=False,
                                            ...
                                            code_dim=32,
                                            **kwargs):
    encoder_config, decoder_config = get_model_default_params(
    ...
    if as_tokenzer:
        ...
        for k in keys:
            if k.startswith("loss") or k.startswith("teacher") or k.startswith(
                    "scaling"):
                del weights[k]
...
@register_model
def vqkd_encoder_base_decoder_3x768x12_clip(pretrained=False,
                                            pretrained_weight=None,
                                            as_tokenzer=False,
                                            img_size=224,
                                            n_code=8192,
...
@register_model
def vqkd_encoder_base_decoder_1x768x12_dino(pretrained=False,
                                            pretrained_weight=None,
                                            as_tokenzer=False,
                                            img_size=224,
                                            n_code=8192,
