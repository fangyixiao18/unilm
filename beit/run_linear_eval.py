...
def eval_linear(args):
    ...
    mean = (0.485, 0.456,
            0.406) if args.imagenet_default_mean_and_std else (0.5, 0.5, 0.5)
    std = (0.229, 0.224,
           0.225) if args.imagenet_default_mean_and_std else (0.5, 0.5, 0.5)
    ...
    # set optimizer
    learning_rate = args.lr or args.base_lr * (args.batch_size_per_gpu *
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train(model, linear_classifier, optimizer, train_loader,
                            epoch, args.avgpool_patchtokens, args.amp_forward)
        scheduler.step()
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            ...
            for classifier_key in test_stats:
                ...
                print(
                    f"Accuracy at epoch {epoch} of the network on the {len(dataset_val)} test images: {classifier['acc1']:.1f}%"
                )
...
def train(model, linear_classifier, optimizer, loader, epoch, avgpool,
          amp_forward):
...
@torch.no_grad()
def validate_network(val_loader, model, linear_classifier, avgpool,
                     amp_forward):
    linear_classifier.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    assert avgpool
    module = linear_classifier.module if hasattr(
        linear_classifier, 'module') else linear_classifier
    for inp, target in metric_logger.log_every(val_loader, 20, header):
        ...
        for i, output in enumerate(all_output):
            ...
            post_str = '_layer%d' % i
            metric_logger.update(loss=loss.item())
            metric_logger.meters['acc1' + post_str].update(
                acc1.item(), n=batch_size)
            if module.num_labels >= 5:
                metric_logger.meters['acc5' + post_str].update(
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=bool_flag,
        help=
        """Whether ot not to concatenate the global average pooled features to the [CLS] token.
    ...
    parser.add_argument(
        '--optimizer', default="adamw", type=str, help='optimizer type')
    parser.add_argument(
        '--pretrained_weights',
        default='',
        type=str,
    ...
    parser.add_argument(
        ...
        type=bool_flag,
        help=
        """Set True to use the imagenet default mean and std, Set False will use the mean and std in Inception. 
    ...
    parser.add_argument(
        ...
        type=bool_flag,
        help=
        'Use amp to inference the pre-trained model, which can speed up the evaluation. '
    )
